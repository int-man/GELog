...
from model_new import Model_noenchanced
import os
...
args.log_name = 'Thunderbird_mini'
args.num_layers = 1
...
training_data = np.load(
    f'./preprocessed_data/{args.log_name}_training_prompt4(temp)_new_data.npz', allow_pickle=True)
# load test data Hdfs
# testing_data = np.load(
#     f'./preprocessed_data/{args.log_name}_testing_data.npz', allow_pickle=True)
testing_data = np.load(
    f'./preprocessed_data/{args.log_name}_testing_prompt4(temp)_new_data.npz', allow_pickle=True)

# num = int(len(training_data['x'])*0.1/8)
# print(num)
...
# x_train, y_train, z_train = training_data['x'][:num], training_data['y'][:num], training_data['z'][:num]
# x_test1, y_test1, z_test1 = training_data['x'][num:], training_data['y'][num:], training_data['z'][num:]
# x_test2, y_test2, z_test2 = testing_data['x'], testing_data['y'], testing_data['z']
# x_test = np.concatenate((x_test1, x_test2), axis=0)
# y_test = np.concatenate((y_test1, y_test2), axis=0)
# z_test = np.concatenate((z_test1, z_test2), axis=0)
# print(len(x_train))
# print(len(x_test1),len(x_test2),len(x_test))

# x_train = 0.5*x_train+0.5*z_train
# x_test = 0.5*x_test+0.5*z_test
...
train_generator = DataGenerator_new(x_train, y_train, z_train, args.window_size)
...
model = Model_noenchanced(num_layers=args.num_layers, dim=EMBEDDING_DIM, window_size=args.window_size, nhead=8, dim_feedforward=4 *
              EMBEDDING_DIM, dropout=0.1)


# fine tuning setting
# if args.tune_mode == 'adapter':
#     print("Tuning adapter")
#     model.train_adapter()
...
for epoch in range(start_epoch+1, epochs):
    ...
    print("epoch : {}/{}, loss = {:.7f}".format(epoch, epochs, train_loss))
...
#------------------------------------------------
#BGL
# Precision: 0.9653  0.9653   0.9653
# Recall: 0.9179     0.9179   0.9179
# F1 score: 0.9410   0.9410   0.9410

# Precision: 1.0000   1.0000  1.0000
# Recall: 0.9734      0.9734  0.9734
# F1 score: 0.9865    0.9865  0.9865
#------------------------------------------------
#HDFS
#Precision: 1.000
#Recall:    0.9946
...
#[0.10282350279854334, 0.0007624141454233449, 0.0006306782373027545, 0.0004341293518728071, 0.0003118718401517299, 0.00030263030473720847, 0.00027295865744088815, 0.00023257815435479416, 0.00019621327730431177, 0.0001900694357388568, 0.00012930722318870948, 0.00011912369553042708, 0.00011255475727123553, 8.571790156541085e-05, 6.176642313772916e-05, 5.041954424574199e-05, 4.620176484593719e-05, 3.945205386485048e-05, 3.120821758657065e-05, 2.817225785409985e-05]
#[0.9921434327155519, 0.9938477054967222, 0.9959555106167847, 0.9968664712422926, 0.9975757575757576, 0.9954778414229726, 0.9972779514063917, 0.9973774460359088, 0.998587855557797, 0.995775497887749, 0.9984862246442628, 0.9986885907394331, 0.9967787396819006, 0.9983867715265174, 0.9983867715265174, 0.9987893462469734, 0.9987893462469734, 0.9987893462469734, 0.9987893462469734, 0.9987893462469734]
